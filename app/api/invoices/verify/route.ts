...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (!qr) return NextResponse.json({ error: 'Missing qr payload' }, { status: 400 });

    const secret = process.env.INVOICE_QR_SECRET || '';
    if (!secret) return NextResponse.json({ error: 'Server misconfigured: INVOICE_QR_SECRET not set' }, { status: 500 });
    ...
    try {
      ...
      console.debug('QR verify: received payload length', qrClean.length, 'sample', qrClean.slice(0, 60));
      ...
      return NextResponse.json({ error: 'Failed to decrypt/parse QR payload' }, { status: 400 });
    }

    // Expect minimal payload: { b: bookingId, t: timestamp }
    const bookingId = decodedJson?.b;
    if (!bookingId) return NextResponse.json({ error: 'Invalid QR payload (missing booking id)' }, { status: 400 });
    ...
    if (!isAdminUser && !isVenueManager) {
      return NextResponse.json({ error: 'Forbidden: caller not authorized' }, { status: 403 });
    }
